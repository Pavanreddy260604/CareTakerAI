...
export function VoiceLogger({ onUpdate }: VoiceLoggerProps) {
    ...
    const processVoice = async (text: string) => {
        ...
        try {
            toast({ title: 'Processing...', description: `"${text}"` });
            const response = await api.parseVoiceText(text);

            if (response.health) {
                onUpdate(response.health);
                toast({ title: 'Logged via Voice', description: 'Health metrics updated.' });
            } else {
                toast({ title: 'No Data Found', description: 'Could not extract health info.' });
            }
        } catch (error) {
